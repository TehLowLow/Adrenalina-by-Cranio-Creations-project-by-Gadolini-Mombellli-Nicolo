...
public class Map2BuilderTest {
    ...
    public void build() {
        ...
        for(Cell cell : map.getYellowRoom().getCells()){
            ...
            if(cell.getName().equalsIgnoreCase("lootCell1")){
                assertTrue(cell.getLeftConnection().getConnectedCell().getName().equalsIgnoreCase("lootCell"));
            }
        }

        assertEquals(map.getBlueRoom().getCells().size(), 3);

        for(Cell cell : map.getBlueRoom().getCells()){

            if(cell.getName().equalsIgnoreCase("lootCell1")){
                assertTrue(cell.getDownConnection().getConnectedCell().getName().equalsIgnoreCase("spawnCell"));
            }
        ...
        assertEquals(map.getWhiteRoom().getCells().size(), 1);
        ...
        for(Cell cell : map.getWhiteRoom().getCells()){
            ...
            if(cell.getName().equalsIgnoreCase("lootCell")){
                assertTrue(cell.getRightConnection().getConnectedCell().getName().equalsIgnoreCase("lootCell3"));
            }
